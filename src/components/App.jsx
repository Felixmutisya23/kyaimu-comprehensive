...
export default function App() {
    ...
    async function loadPublicData() {
      try {
        ...
        if (!error && rows && rows.length > 0) {
          ...
          setPublicPageData(schoolData || { notFound: true, slug });
        } else {
  ...
  // ── Public school page — show WITHOUT login ─────────────────
  if (isPublicRoute) {
    if (publicPageLoading) return (
      <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#f8fafc', fontFamily:'sans-serif' }}>
        <div style={{ textAlign:'center' }}>
          ...
          <div style={{ fontSize:16, color:'#64748b', fontWeight:600 }}>Loading school page...</div>
        </div>
      </div>
    );
    if (publicPageData?.notFound) return (
      <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#f8fafc', fontFamily:'sans-serif' }}>
        <div style={{ textAlign:'center', maxWidth:400, padding:24 }}>
          <div style={{ fontSize:64, marginBottom:16 }}>🔍</div>
          <div style={{ fontSize:24, fontWeight:900, color:'#1e293b', marginBottom:8 }}>School not found</div>
          <div style={{ fontSize:14, color:'#64748b', marginBottom:24, lineHeight:1.7 }}>
            No school found at this link. It may be incorrect or the school may not have set up their public page yet.
          </div>
          <a href="/" style={{ padding:'12px 28px', background:'#1e40af', color:'#fff', borderRadius:10, textDecoration:'none', fontWeight:700, fontSize:15 }}>
            Back to Login
          </a>
        </div>
      </div>
    );
    if (publicPageData) return (
      <PublicPage data={publicPageData} onLoginClick={() => { window.location.href = '/'; }} />
    );
    return (
