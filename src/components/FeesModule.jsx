...
/* ═══════════════════════════════════════════════════════
   ...
   STUDENT CATEGORY LOGIC
   ────────────────────────────────────────────────────────
   Each school can have one or more "student categories":
     - Day Scholar   (always present)
     - Boarder       (optional – if school has boarding)
   
   A student record carries a `category` field: 'day' | 'boarding' | null
   (null = treated as 'day' for legacy data)
   
   Fee types can be tagged with an applicableCategories array:
     [] / undefined = applies to ALL categories
     ['boarding']   = boarding students only
     ['day']        = day scholars only
     ['day','boarding'] = both (same as all)
   
   feeSchedule entries follow the same pattern but can also
   be scoped per category when the school needs different
   amounts for the same fee type (e.g. Activity Fee KES 2000 for
   day, KES 3500 for boarders).  The schedule entry gains an
   optional `category` field: 'ALL' | 'day' | 'boarding'
═══════════════════════════════════════════════════════ */

const METHODS  = ['Mpesa', 'Bank Transfer', 'Cash', 'Cheque'];
const TERMS    = ['1', '2', '3'];
const CATS     = ['day', 'boarding'];
const CAT_LABELS = { day: 'Day Scholar', boarding: 'Boarder' };
...
function findSchedule(student, feeTypeId, term, year, data) {
  ...
  // prefer category-specific
  const specific = all.find(s => s.category == cat);
  if (specific) return specific;
  // fall back to ALL
  return all.find(s => !s.category || s.category == 'ALL') || null;
}
...
function schoolHasBoarding(data) {
  return data.schoolBoardingMode === 'boarding_only' ||
...
function printFeeList(students, feeTypeId, term, year, data, feeTypeName) {
  const rows = students.map((s, i) => {
    ...
    const catLabel = CAT_LABELS[studentCategory(s)] || 'Day Scholar';
    return `<tr style="background:${i%2==0?'#fff':'#f8f9ff'}">
      ...
      <td style="padding:5px 8px;border:1px solid #ddd;text-align:center;font-size:11px">${catLabel}</td>
  ...
  <table style="width:100%;border-collapse:collapse;font-size:11px">
    <thead><tr style="background:#003399;color:#fff">
      ...
      <th style="padding:6px 8px;border:1px solid #003399">CATEGORY</th>
...
function printStudentStatement(student, data) {
  ...
  const catLabel = CAT_LABELS[studentCategory(student)] || 'Day Scholar';
  const w = window.open('', '_blank');
  w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8">
  ...
  <table style="width:100%;border-collapse:collapse;margin-bottom:14px;font-size:12px">
    ...
    <tr><td style="padding:5px 8px;border:1px solid #ddd;background:#f0f4ff;font-weight:700">Class (Current)</td><td style="padding:5px 8px;border:1px solid #ddd">${student.class}</td>
        <td style="padding:5px 8px;border:1px solid #ddd;background:#f0f4ff;font-weight:700">Category</td><td style="padding:5px 8px;border:1px solid #ddd;font-weight:700">${catLabel}</td></tr>
...
function printFeeStructure(classes, data, selClass) {
  ...
  const hasBoarding   = schoolHasBoarding(data);
  ...
  const tableRows = targetClasses.map(cls => {
    ...
    let rows = '';
    let dayTotal = 0, boardTotal = 0;
    classTypes.forEach(ft => {
      [1,2,3].forEach(term => {
        const yr = new Date().getFullYear();
        const dayScheds  = schedule.filter(s => s.feeTypeId==ft.id && (s.class==cls||s.class=='ALL') && s.term==term && s.year==yr && (!s.category||s.category=='ALL'||s.category=='day'));
        const boardScheds= schedule.filter(s => s.feeTypeId==ft.id && (s.class==cls||s.class=='ALL') && s.term==term && s.year==yr && (s.category=='boarding'));
        const dayAmt   = dayScheds.find(s=>s.category=='day')?.amount  ?? dayScheds.find(s=>!s.category||s.category=='ALL')?.amount ?? 0;
        const boardAmt = boardScheds[0]?.amount ?? dayAmt;
        dayTotal   += Number(dayAmt);
        boardTotal += Number(boardAmt);
    ...
    return `
      <tr style="background:#003399;color:#fff">
        <td colspan="${hasBoarding?5:4}" style="padding:8px;font-weight:900;font-size:14px;border:1px solid #003399">CLASS: ${cls.toUpperCase()}</td>
      </tr>
      ${rows}
      <tr style="background:#e8f0ff">
        <td colspan="${hasBoarding?2:2}" style="padding:6px 8px;border:1px solid #ddd;font-weight:700;text-align:right">Total Annual Fees (Day):</td>
        <td style="padding:6px 8px;border:1px solid #ddd"></td>
        <td style="padding:6px 8px;border:1px solid #ddd;text-align:right;font-weight:900;font-size:14px;color:#003399">KES ${dayTotal.toLocaleString()}</td>
        ${hasBoarding?`<td style="padding:6px 8px;border:1px solid #ddd;text-align:right;font-weight:900;font-size:14px;color:#7c3aed">KES ${boardTotal.toLocaleString()}</td>`:''}
      </tr>
      <tr><td colspan="${hasBoarding?5:4}" style="padding:4px;border:none"></td></tr>`;
  }).join('');

  const w = window.open('', '_blank');
  w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8">
  <title>Fee Structure ${new Date().getFullYear()} — ${data.schoolName}</title>
  <style>body{font-family:Arial,sans-serif;padding:24px;max-width:900px;margin:0 auto;font-size:12px}
  ...
  <table style="width:100%;border-collapse:collapse">
    <thead><tr style="background:#1e3a8a;color:#fff">
      ...
      <th style="padding:7px 8px;border:1px solid #1e3a8a;text-align:right">Day Scholar (KES)</th>
      ${hasBoarding?'<th style="padding:7px 8px;border:1px solid #1e3a8a;text-align:right">Boarder (KES)</th>':''}
    </tr></thead>
...
function FeeOverview({ data, setData, user, canManage }) {
  ...
  const filtered = activeStudents.filter(s =>
    ...
    (!filterCategory || studentCategory(s) == filterCategory) &&
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {hasBoarding && (
          <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)} style={{ width: 140 }}>
      ...
      <Card style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={TS.table}>
            <thead><tr>
              {['Adm No','Name','Class', ...(hasBoarding?['Category']:[]),'Expected','Paid','Balance','Status','Actions'].map(h =>
                ...
                : filtered.map(s => {
                  ...
                  return (
                    <tr key={s.id}>
                      ...
                      {hasBoarding && (
                        <td style={TS.td}>
                          <Tag color={cat=='boarding'?'purple':'blue'}>
                            {cat=='boarding'?'🏠 Boarder':'🚶 Day'}
                          </Tag>
                        </td>
                      )}
...
function ByFeeType({ data, setData, user, canManage }) {
  ...
  const eligibleStudents = useMemo(() => {
    ...
    let students = ft
      ? (ft.appliesToAll ? active : active.filter(s => (ft.applicableClasses||[]).includes(s.class)))
      : active;
    // filter by category — the fee type may also restrict categories
    if (ft) students = students.filter(s => feeTypeAppliesTo(ft, studentCategory(s)));
    if (selClass) students = students.filter(s => s.class == selClass);
    if (selCat)   students = students.filter(s => studentCategory(s) == selCat);
    return students;
  }, [ft, selClass, selCat, data.students]);
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {hasBoarding && (
          <select value={selCat} onChange={e => setSelCat(e.target.value)} style={{ width: 140 }}>
            <option value="">All Categories</option>
            <option value="day">Day Scholars</option>
            <option value="boarding">Boarders</option>
          </select>
        ...
        <>
          ...
          <Card style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ overflowX: 'auto' }}>
              <table style={TS.table}>
                <thead><tr>
                  {['Adm No','Name','Class',...(hasBoarding?['Category']:[]),'Expected','Paid','Balance','Status'].map(h =>
                    <th key={h} style={TS.th}>{h}</th>)}
                </tr></thead>
                <tbody>
...
function RecordPaymentModal({ show, onClose, data, setData, defaultStudent }) {
  ...
  // When student is selected, show the expected amount for the chosen fee type
  const selStudent = (data.students || []).find(s => s.id == Number(form.studentId));
  const expectedAmt = selStudent && form.feeTypeId
    ? getExpected(selStudent, form.feeTypeId, form.term, form.year, data)
    : null;

  // Only show fee types that apply to this student's category
  const applicableFeeTypes = (data.feeTypes || []).filter(ft => {
    if (!selStudent) return true;
    return feeTypeAppliesTo(ft, studentCategory(selStudent));
  });
  ...
  return (
    <Modal show={show} onClose={onClose} title="Record Fee Payment">
      <FormGroup label="Student *">
        <select value={form.studentId} onChange={e => setForm({ ...form, studentId: e.target.value, feeTypeId: '' })}>
          <option value="">Choose student...</option>
          {(data.students || []).filter(s => !s.status || s.status == 'active').map(s => (
            <option key={s.id} value={s.id}>{s.name} ({s.admNo}) — {s.class}{s.category=='boarding'?' 🏠':''}</option>
          ))}
        </select>
      </FormGroup>
      ...
      <FormRow>
        <FormGroup label="Amount (KES) *">
          <input type="number" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} placeholder={expectedAmt ? `e.g. ${expectedAmt}` : 'e.g. 5000'} />
        </FormGroup>
...
function FeeStructure({ data, setData }) {
  ...
  const [schedForm, setSchedForm] = useState({
    ...
    term: '1', year: String(curYear()), amount: '',
  });
  ...
  const allClasses  = getAllClasses(data);
  const hasBoarding = schoolHasBoarding(data);
  const boardingMode= data.schoolBoardingMode || 'day_only';

  function saveBoardingMode(mode) {
    setData(d => ({ ...d, schoolBoardingMode: mode }));
    setShowBoardingSetup(false);
  }
  ...
  function addSched() {
    ...
    setSchedForm({ feeTypeId: '', class: 'ALL', category: 'ALL', term: '1', year: String(curYear()), amount: '' });
  }
  ...
  const modeLabels = {
    ...
    boarding_only:  'Boarding School Only',
    both:           'Day & Boarding (Mixed)',
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, background: '#1e2435', border: '1px solid #2a3350', borderRadius: 10, padding: '10px 16px' }}>
        ...
        <Btn size="sm" variant="ghost" onClick={() => setShowBoardingSetup(true)}>⚙ Change</Btn>
      </div>

      <Alert type="info">
        <Icon name="alert" size={14} />
        {hasBoarding
          ? ' This school has both Day and Boarding students. You can set different amounts for the same fee type per category. Fee types can also be restricted to Day-only or Boarders-only.'
              ...
              <div key={ft.id} style={{ padding: '10px 0', borderBottom: '1px solid #2a3350' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    ...
                    {hasBoarding && (
                      <Tag color={
                        !ft.applicableCategories || ft.applicableCategories.length==0 ? 'gray' :
                        ft.applicableCategories.includes('boarding') && !ft.applicableCategories.includes('day') ? 'purple' :
                        ft.applicableCategories.includes('day') && !ft.applicableCategories.includes('boarding') ? 'blue' : 'gray'
                      }>
                        {!ft.applicableCategories || ft.applicableCategories.length==0 ? 'All Students' :
                         ft.applicableCategories.includes('boarding') && !ft.applicableCategories.includes('day') ? '🏠 Boarders only' :
                         ft.applicableCategories.includes('day') && !ft.applicableCategories.includes('boarding') ? '🚶 Day only' : 'All Students'}
                      </Tag>
                    )}
          ...
          {feeSchedule.length == 0
            ...
            : (
              <div style={{ overflowX: 'auto' }}>
                <table style={TS.table}>
                  <thead><tr>
                    {['Fee Type','Class',...(hasBoarding?['For']:[]),'Tm','Year','Amount',''].map(h =>
                      <th key={h} style={TS.th}>{h}</th>)}
                  </tr></thead>
                  <tbody>
                    {feeSchedule.map(sch => {
                      ...
                      return (
                        <tr key={sch.id}>
                          ...
                          {hasBoarding && (
                            <td style={TS.td}>
                              <Tag color={sch.category=='boarding'?'purple':sch.category=='day'?'blue':'gray'}>
                                {sch.category=='boarding'?'🏠 Boarders':sch.category=='day'?'🚶 Day':'All'}
                              </Tag>
                            </td>
                          )}
      ...
      <Modal show={showBoardingSetup} onClose={() => setShowBoardingSetup(false)} title="School Boarding Configuration">
        ...
        {[
          { id: 'day_only',       icon: '🚶', title: 'Day School Only', desc: 'All students are day scholars. No boarding fees. One fee amount per fee type per class.' },
          { id: 'boarding_only',  icon: '🏠', title: 'Boarding School Only', desc: 'All students board. One fee amount per fee type per class (boarding amounts).' },
          { id: 'both',           icon: '🏠🚶', title: 'Mixed — Day & Boarding', desc: 'The school has both day scholars and boarders. Students in the same class may pay different amounts. Each fee type can have a different amount for day vs boarding, and some fees (e.g. Boarding Charges) can apply to boarders only.' },
        ].map(opt => (
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8 }}>
          <Btn variant="ghost" onClick={() => setShowBoardingSetup(false)}>Close</Btn>
        </div>
      </Modal>
      ...
      <Modal show={showAddType} onClose={() => setShowAddType(false)} title="Add Fee Type">
        <FormGroup label="Fee Type Name *">
          <input value={typeForm.name} onChange={e => setTypeForm({ ...typeForm, name: e.target.value })}
            placeholder="e.g. Tuition Fee, Boarding Charges, Sports Fee, Building Fund" autoFocus />
        </FormGroup>
        ...
        {/* Category applicability — only shown if school has boarding */}
        {hasBoarding && (
          <FormGroup label="Who does this fee apply to?">
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {[
                { id: [],              label: 'All Students (Day & Boarding)' },
                { id: ['day'],         label: '🚶 Day Scholars only' },
                { id: ['boarding'],    label: '🏠 Boarders only' },
              ].map(opt => (
