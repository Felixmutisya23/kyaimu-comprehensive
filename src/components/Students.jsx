...
export default function Students({ data, setData, user, isUnlocked = true }) {
  ...
  function save() {
    ...
    // Fees start at zero — finance office handles fee assignment separately
    const ns = {
      id: Date.now(),
      ...form,
      stream,
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          ...
          <Card>
            ...
            {[['Parent/Guardian', s.parent], ['Phone', s.phone], ['Date of Birth', s.dob],
              ['Year Joined', s.joined], ['Current Class', s.class]]
              .map(([l, v]) => (
            ...
            <Card>
              ...
              {s.fees.total === 0 ? (
                <div style={{ color: '#64748b', fontSize: 13 }}>
                  No fee record set for this student yet. Go to the Fees module to assign fees.
                </div>
              ) : (
                <>
              ...
              return (
                <div key={ex.id} style={{ marginBottom: 16 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                    ...
                    {(isPrincipal || isClassTeacher) && (
                      <span style={{ color: g.color, fontWeight: 700 }}>Mean: {g.label} ({mean})</span>
                    )}
                  </div>
                  <div style={{ overflowX: 'auto' }}>
                    <table style={TS.table}>
                      <thead>
                        <tr>
                          {[...visibleSubs, ...(isPrincipal || isClassTeacher ? ['Total', 'Grade'] : [])].map(h => (
                      ...
                      <tbody>
                        <tr>
                          ...
                          {(isPrincipal || isClassTeacher) && (
                            <>
                              <td style={{ ...TS.td, fontWeight: 700 }}>{total}</td>
                              <td style={TS.td}><GradeBadge score={mean} /></td>
                            </>
                          )}
                        </tr>
                      </tbody>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, alignItems: 'center', flexWrap: 'wrap' }}>
        <input placeholder="Search name or adm no..." value={search}
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={TS.table}>
            ...
            <tbody>
              {filtered.map(s => {
                ...
                return (
                  <tr key={s.id}>
                    ...
                    <td style={TS.td}>
                      {s.cases.length > 0
                        ? <Tag color="red">{s.cases.length}</Tag>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7} style={{ ...TS.td, textAlign: 'center', color: '#64748b', padding: 28 }}>
                    No students found.
                  </td>
                </tr>
              )}
      ...
      {/* Add Student Modal — no fee fields */}
      <Modal show={show} onClose={() => { setShow(false); setForm(initForm()); }} title="Add New Student">
        <FormRow>
          <FormGroup label="Full Name">
            <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Alice Muthoni" autoFocus />
          </FormGroup>
          <FormGroup label="Admission Number">
            <input value={form.admNo} onChange={e => setForm({ ...form, admNo: e.target.value })} placeholder="e.g. ADM009" />
          </FormGroup>
        </FormRow>
        <FormRow>
        ...
        {isClassTeacher && !isPrincipal && (
          <Alert type="info">
            ...
            This student will be added to your class: <strong>{myClass}</strong>
          </Alert>
        )}
        <Alert type="info">
          <Icon name="alert" size={13} />
          Fee details are managed in the Fees module after the student is enrolled.
        </Alert>
